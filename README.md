...
- **現在の到達点**:
  停止状態の維持は確認できていますが、再生ボタン1回で確実に映像が復帰するかどうかは環境やタイミングに依存します。後述のdogfood統合版において、切替中の論理状態を引き継ぐことで復帰率の改善を試みていますが、停止していた位置は失われ、最新時刻への再構築が発生します。
...
> **配備版に関する注意**: 実機測定を行った配備版（KonomiTV source `e6d9cf7`、DPlayer `2499f05`）は、現在公開しているforkの[`dogfood/integration`](https://github.com/libratechw/KonomiTV/tree/dogfood/integration)に含まれます。ただし、配備イメージは配備時点の固定ビルドであり、branch上のREADME更新が既存イメージへ遡って反映されるわけではありません。以下の測定結果は記載した特定ビルド・条件に限定され、branch全体や公開先端の将来の変更を保証するものではありません。
...
- **新測定版（dogfood配備: KonomiTV `e6d9cf7` / DPlayer `2499f05`）**:
...
- dogfood配備版（KonomiTV `e6d9cf7` / DPlayer `2499f05`）を用い、POCOのChromeで同一録画をOriginal画質で3回再生しました。
